...
これまで、 ノートPCがサスペンド状態に入ると、有線マウスやBluetoothキーボードの操作をしてもサスペンド状態から復帰せず、
わざわざ、ノートPCの蓋を開けてサスペンドから復帰させる必要ありました。
クラムシェルモードでの使用において非常に不便でした。

今回、USB/Bluetooth 接続のマウスやキーボード操作だけでサスペンド状態から復帰できるよう設定を変更したので、その手順を共有します。
...
このキーボードは、 `WakeAllowed: yes` でウェイクアップが許可されています。
...
`hardware-configuration.nix` に、以下の `boot.kernelParms = ... ` を追加します。
...
私の環境では、
USB 外付 SSD `Bus 002 Device 002: ID 04bb:01a0 I-O Data Device, Inc. I-O DATA SSPJ-UTC` を接続しており、
ここに、 NixOS システムをインストールしています。
そのため、サスペンド状態にもかかわらず、OS の ディスクアクセスにより、サスペンドが解除され、外部モニターが点灯するという副作用が発生しました。

そこで、ウェィクアップの要因を、HIDデバイス(マウス・キーボード)と Bluetooth 機器 のみに限定します。
...
解決に至る過程で、Linxu の電源管理や udev ルールの仕組みなど学ぶことが多く、非常に有意義なカスタマイズとなりました。
